...
public class Ari : MonoBehaviour
{
    ...
    void Move()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            iTween.MoveTo(this.gameObject,  iTween.Hash("x", position.x, "y", position.y, "time", 20.0f));
            flg = 1;        
        }
        if(flg == 1){
            this.transform.rotation = Quaternion.FromToRotation (Vector3.up, diff);    
        ...
        if(count == 0){
            ...
            // 自動で前進する。
            iTween.MoveTo(this.gameObject,  iTween.Hash("x", pos_x, "y", pos_y, "time", 300.0f));
