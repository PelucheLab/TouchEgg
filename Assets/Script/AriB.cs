...
public class AriB : MonoBehaviour
{

    Animator animator;
    float chargeTime = 8.0f;
    float timeCount;
    float pos_x = 0.0f;
    float pos_y = 0.0f;
    int flg = 0;
    ...
    void Move()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            iTween.MoveTo(this.gameObject,  iTween.Hash("x", position.x, "y", position.y, "time", 30.0f));
            flg = 1;
        ...
        if(count == 0){
            ...
            // 自動で前進する。
            iTween.MoveTo(this.gameObject,  iTween.Hash("x", pos_x, "y", pos_y, "time", 400.0f));
