...
public class AriC : MonoBehaviour
{

    Animator animator;
    float chargeTime = 10.0f;
    float timeCount;
    float pos_x = 1000.0f;
    float pos_y = 0.0f;
    int flg = 0;
    ...
    void Move()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            iTween.MoveTo(this.gameObject,  iTween.Hash("x", position.x, "y", position.y, "time", 30.0f,"delay",0.5f));
            flg = 1;
        ...
        if(count == 0){
            ...
            // 自動で前進する。
            iTween.MoveTo(this.gameObject,  iTween.Hash("x", pos_x, "y", pos_y, "time", 800.0f));
